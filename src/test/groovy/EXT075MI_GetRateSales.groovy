 ...
 *
 * Name : EXT075MI.GetRateSales
 * Description : The GetRateSales transaction get records to the EXT075 table.
...
public class GetRateSales extends ExtendM3Transaction {
  ...
  public GetRateSales(MIAPI mi, DatabaseAPI database, LoggerAPI logger, ProgramAPI program, UtilityAPI utility) {
    this.mi = mi
  ...
  public void main() {
    ...
    if(iCUNO==null || iCUNO==""){
      ...
      if(!queryEXT080.readAll(EXT080, 4, outDataEXT080)){
        mi.error("L'enregistrement EXT080 n'existe pas")
        return
      }
      if(iCUNO==null || iCUNO==""){
    ...
    EXT075.set("EXOBV1", obv1.trim())
    EXT075.set("EXOBV2", obv2.trim())
    EXT075.set("EXOBV3", obv3.trim())
