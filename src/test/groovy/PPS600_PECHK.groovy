 ...
 * 20240708     YVOYOU       CMD61 : Add sale point control multi OA
 */
...
public class PPS600_PECHK extends ExtendM3Trigger {
  ...
  public void main() {
    ...
    //Sale point control
    if (WFPUNO != WTPUNO) {
      ExpressionFactory expression_MPHEAD = database.getExpressionFactory("MPHEAD")
      expression_MPHEAD = expression_MPHEAD.ge("IAPUNO", WFPUNO)
      expression_MPHEAD = expression_MPHEAD.and(expression_MPHEAD.le("IAPUNO", WTPUNO))
      DBAction MPHEAD_queryFull = database.table("MPHEAD").index("00").matching(expression_MPHEAD).selection("IAFACI", "IAWHLO", "IARASN", "IAORTY", "IAPUSL", "IABUYE", "IAPURC", "IASUNO").build()
      DBContainer MPHEADFull = MPHEAD_queryFull.getContainer()
      MPHEADFull.set("IACONO", (Integer)program.getLDAZD().CONO)
      if (!MPHEAD_queryFull.readAll(MPHEADFull, 1, maxRecord, outData_MPHEAD_Full)) {
      }
      if (IN60) {
        interactive.showCustomError("WFPUNO", "Un ou plusieurs OAs ne peuvent pas être imprimés, il manque l'affectation point de vente")
        return
      }
    }else{
  ...
  // Get MPHEAD
  Closure<?> outData_MPHEAD_Full = { DBContainer MPHEAD ->
    faci = MPHEAD.get("IAFACI")
    whlo = MPHEAD.get("IAWHLO")
    rasn = MPHEAD.get("IARASN")
    orty = MPHEAD.get("IAORTY")
    ...
    purc = MPHEAD.get("IAPURC")
    suno = MPHEAD.get("IASUNO")
    punoFull = MPHEAD.get("IAPUNO")
    if (suno.startsWith("BA")) {
      //PO line number control
      ExpressionFactory expression_MPLINE_Full = database.getExpressionFactory("MPLINE")
      expression_MPLINE_Full = expression_MPLINE_Full.le("IBELNO", " ")
      DBAction MPLINE_query_Full = database.table("MPLINE").index("00").matching(expression_MPLINE_Full).selection("IBPNLI", "IBDWDT").build()
      DBContainer MPLINEFull = MPLINE_query_Full.getContainer()
      MPLINEFull.set("IBCONO", (Integer)program.getLDAZD().CONO)
      MPLINEFull.set("IBPUNO", punoFull)
      if (MPLINE_query_Full.readAll(MPLINEFull, 2, 1000, outData_MPLINE)) {
        IN60 = true
      }
    }
  }
