 ...
 *
 * Name : EXT052
 * Description : Select the items based on the criteria contained in EXT051 table and add records to the EXT052 table (EXT052MI.SelAssortItems conversion)
 * Date         Changed By   Description
 * 20210421     RENARN       TARX02 - Add assortment
 * 20211027     RENARN       Added new input parameters, allow read MITMAS with CONO, added update of EXT080/EXT081, added expression on reading MPAGRL
 * 20211209     YVOYOU       EXT800 management has been added
 * 20211220     RENARN       Added criterias and filters
 * 20220208     RENARN       New blocking constraints have been added
 * 20220420     RENARN       Replace executeEXT053MIAddAssortItems by executeEXT820MISubmitBatch/EXT053
 * 20220601		  RENARN		   Exclusion criteria added (fuds)
 * 20221013		  YOUYVO		   Add Business Chain Management
 * 20230223     RENARN       Check selection header message has been modified
 * 20231128		  SEAR		     ITTY criteria not unique
 */

import java.time.LocalDateTime
import java.time.format.DateTimeFormatter

public class EXT052 extends ExtendM3Batch {
  ...
  public EXT052(LoggerAPI logger, DatabaseAPI database, UtilityAPI utility, ProgramAPI program, BatchAPI batch, MICallerAPI miCaller, TextFilesAPI textFiles) {
    this.logger = logger
  ...
  private performActualJob(Optional<String> data){
    ...
    if(!query.readAll(EXT050, 4, outData_EXT050)){
      String header = "MSG;"+"ASCD;"+"CUNO;"+"DAT1"
    ...
    if(criteria_found){
      ...
      if (inITNO == null || inITNO.trim().isBlank()) {
        if (itty != "") {
          ...
          DBAction MITMAS_query = database.table("MITMAS").index("70").matching(expression_MITMAS).selection("MMITTY", "MMBUAR", "MMHIE1", "MMHIE2", "MMHIE3", "MMHIE4", "MMHIE5", "MMCFI1", "MMITGR", "MMCFI5", "MMSTAT", "MMCFI4", "MMHAZI", "MMFUDS").build()
        ...
        DBAction MITMAS_query = database.table("MITMAS").index("00").selection("MMITTY", "MMBUAR", "MMHIE1", "MMHIE2", "MMHIE3", "MMHIE4", "MMHIE5", "MMCFI1", "MMITGR", "MMCFI5", "MMSTAT", "MMCFI4", "MMHAZI", "MMFUDS").build()
        DBContainer MITMAS = MITMAS_query.getContainer()
        MITMAS.set("MMCONO", currentCompany)
      ...
      if (inOPT2 == "1") {
        ...
        executeEXT820MISubmitBatch(currentCompany as String, "EXT053", ascd, cuno, fdat, "", "", "", "", "")
      }
      // Update mode
      if (inOPT2 == "2") {
 ...
 */
  public boolean itemSelectionOK() {
    ...
    if(fuds.substring(0, 2) == "S/" ||
      fuds.substring(0, 4) == "SUB/" ||
      fuds.substring(0, 3) == "PL/" ||
      fuds.substring(0, 3) == "GP/" ||
      fuds.substring(0, 3) == "GN/" ||
      fuds.substring(0, 3) == "PS/" ||
      fuds.substring(0, 3) == "PN/" ||
      fuds.substring(0, 3) == "NT/" ||
      fuds.substring(0, 3) == "MN/" ||
      fuds.substring(0, 4) == "ELP/"){
      return false
    }
    DBAction EXT051_query = database.table("EXT051").index("00").selection("EXCHB1").build()
    DBContainer EXT051 = EXT051_query.getContainer()
 ...
 */
  public boolean constraintsOK() {
    ...
    if (MITBAL_query.read(MITBAL)) {
      ...
      logger.debug("SUNO/supplierNumber = " + supplierNumber)
    ...
    if(potentiallyDangerous == "0") {
      ...
      if (CUGEX1_MITHRY_query.read(CUGEX1_MITHRY)) {
        if (CUGEX1_MITHRY.get("F1A330").toString().trim() == "OUI") {
          ...
          logger.debug("ZPDA/potentiallyDangerous = " + potentiallyDangerous)
        }
      }
    }
    dangerClass = ""
    ...
    if(CUGEX1_MITVEN_query.read(CUGEX1_MITVEN)){
      ...
      logger.debug("ZONU/dangerClass = " + dangerClass)
    }
  ...
  Closure<?> outData_EXT010 = { DBContainer EXT010 ->
    logger.debug("constrainte trouvée 1 = " + EXT010.get("EXZCID"))
    ...
    logger.debug("constrainte trouvée 2 = " + EXT010.get("EXZCID"))
  ...
  Closure<?> outData_MPAPMA2 = { DBContainer MPAPMA2 ->
    ...
    if(query.read(CUGEX1)){
      ...
      logger.debug("SPE1/ingredient1 = " + ingredient1)
      logger.debug("SPE2/ingredient2 = " + ingredient2)
      logger.debug("SPE2/ingredient3 = " + ingredient3)
      logger.debug("SPE2/ingredient4 = " + ingredient4)
      logger.debug("SPE2/ingredient5= " + ingredient5)
      logger.debug("SPE2/ingredient6 = " + ingredient6)
      logger.debug("SPE2/ingredient7 = " + ingredient7)
      logger.debug("SPE2/ingredient8 = " + ingredient8)
      logger.debug("SPE2/ingredient9 = " + ingredient9)
      logger.debug("SPE2/ingredient10 = " + ingredient10)
    }
  }
  ...
  private executeEXT053MIAddAssortItems(String ASCD, String CUNO, String FDAT){
    ...
    Closure<?> handler = { Map<String, String> response ->
      if (response.error != null) {
        String header = "MSG"
        String message = "Failed EXT053MI.AddAssortItems: "+ response.errorMessage
        logMessage(header, message)
        return
      } else {
      }
  ...
  private executeEXT053MIUpdAssortItems(String ASCD, String CUNO, String FDAT){
    ...
    Closure<?> handler = { Map<String, String> response ->
      if (response.error != null) {
        String header = "MSG"
        String message = "Failed EXT053MI.UpdAssortItems: "+ response.errorMessage
        logMessage(header, message)
        return
      } else {
      }
  ...
  public void Update_CUGEX1(String status, String count){
    ...
    if (!CUGEX1_query.read(CUGEX1)) {
      logger.debug("ascd = " + ascd)
      logger.debug("cuno = " + cuno)
      logger.debug("fdat = " + fdat)
