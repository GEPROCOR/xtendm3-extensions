public class CRS235_PEUPD extends ExtendM3Trigger {
...
// private final String[] runOnlyForUsers = ["DUVCYR"]  // Leave the array empty if it should be run for everyone, otherwise add authorized usernames
  ...
  public CRS235_PEUPD(InteractiveAPI interactive, ProgramAPI program, LoggerAPI logger, MICallerAPI miCaller, DatabaseAPI database) {
  ...
  public void main() {
    // if (!shouldRun()) return
    //logger.debug("Mode= ${interactive.getMode()}")
    if(!interactive.getMode().equals("add")  && !interactive.getMode().equals("change"))return
    //logger.debug("TypeAdresse= {$interactive.display.fields.WEADTH}")
    if(interactive.display.fields.WEADTH!=4) return
    String ADK1 =interactive.display.fields.WEADK1
    //logger.debug("Adresse= {$ADK1}")

    if(interactive.display.fields.WEADK1.toString().trim().length()>4){
      ADK1=interactive.display.fields.WEADK1.toString().trim().substring(0,4)
    }
    //logger.debug("Adresse= {$ADK1}")
    ...
    while (it.hasNext()) {
      ...
      String iCONM = interactive.display.fields.WECONM;
      //logger.debug("iCONM= {$iCONM}")
      def paramEXT031MI_Add = ["CONO": iCONO, "TEDL": iTEDL, "ZPLA": iZPLA, "ZIPL": iZIPL];
      def paramEXT031MI_Upd = ["CONO": iCONO, "TEDL": iTEDL, "ZPLA": iZPLA, "ZIPL": iZIPL, "CONM": iCONM];
