 ...
 *
 * Name : EXT806MI.LstFileLog
 * Description : List file log
...
public class LstFileLog extends ExtendM3Transaction {
  private final MIAPI mi
  private final DatabaseAPI database
  private final LoggerAPI logger
  private final ProgramAPI program

  public LstFileLog(MIAPI mi, DatabaseAPI database, LoggerAPI logger, ProgramAPI program) {
    this.mi = mi
  ...
  public void main() {
    ...
    if (mi.in.get("ZMMN") == null) {
      DBAction query = database.table("EXT806").index("00").selection("EXZMMN", "EXLMTS", "EXMINM", "EXTRNM", "EXMSID", "EXMSGD", "EXZLIN", "EXZIFN", "EXZIFL", "EXUSID", "EXCOID", "EXSTAT", "EXRGDT", "EXRGTM", "EXLMDT", "EXCHNO", "EXCHID").build()
      DBContainer EXT806 = query.getContainer()
      EXT806.set("EXCONO", currentCompany)
      if(!query.readAll(EXT806, 1, 1000, outData)){
        mi.error("L'enregistrement n'existe pas")
        return
      }
    } else {
      String ZMMN = mi.in.get("ZMMN")
      ExpressionFactory expression = database.getExpressionFactory("EXT806")
      expression = expression.ge("EXZMMN", ZMMN)
      DBAction query = database.table("EXT806").index("00").matching(expression).selection("EXZMMN", "EXLMTS", "EXMINM", "EXTRNM", "EXMSID", "EXMSGD", "EXZLIN", "EXZIFN", "EXZIFL", "EXUSID", "EXCOID", "EXSTAT", "EXRGDT", "EXRGTM", "EXLMDT", "EXCHNO", "EXCHID").build()
      DBContainer EXT806 = query.getContainer()
      EXT806.set("EXCONO", currentCompany)
      EXT806.set("EXZMMN",  mi.in.get("ZMMN"))
      if(!query.readAll(EXT806, 2, 1000, outData)){
        mi.error("L'enregistrement n'existe pas")
        return
      }
    }
  ...
  Closure<?> outData = { DBContainer EXT806 ->
    String ZMMN = EXT806.get("EXZMMN")
    String LMTS = EXT806.get("EXLMTS")
